...
void Statistics::Finalize() {
  ...
  for (uint8_t binGroupNr = 0; binGroupNr < NR_OF_BIN_GROUPS; binGroupNr++) {
    ...
    float nrOfBinsSummedUp = (m_sensorData->binGroup[binGroupNr].lastBin - m_sensorData->binGroup[binGroupNr].firstBin + 1) * m_sensorData->snapshotCtr;
    m_sensorData->binGroup[binGroupNr].magAVG = (float)magSumGroup / nrOfBinsSummedUp;
    m_sensorData->binGroup[binGroupNr].magAVGkorr = (float)magSumGroupKorr / nrOfBinsSummedUp;

    if (binGroupNr < NR_OF_BIN_GROUPS)
    {
